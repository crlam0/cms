...
class Session implements \IteratorAggregate, \ArrayAccess, \Countable
{
    ...
    public function init()
    {
        ...
        if ($this->getIsActive()) {
            App::debug('Session is already started', __METHOD__);
    ...
    public function open()
    {
        ...
        if ($this->getIsActive()) {
            App::debug('Session started', __METHOD__);
            $this->updateFlashCounters();
        } else {
            $error = error_get_last();
            $message = isset($error['message']) ? $error['message'] : 'Failed to start session.';
            App::error($message, __METHOD__);
        }
    }
    ...
    protected function registerSessionHandler()
    {
        ...
        if ($this->handler !== null) {
            ...
            App::$debug ? session_set_save_handler($this->handler, false) : @session_set_save_handler($this->handler, false);
        ...
        } elseif ($sessionModuleName !== static::$_originalSessionModule
    ...
    {
        if ($this->getIsActive()) {
            App::$debug ? session_write_close() : @session_write_close();
        }
    ...
    public function getHasSessionId()
    {
        if ($this->_hasSessionId === null) {
            ...
            if (!empty($_COOKIE[$name]) && ini_get('session.use_cookies')) {
                ...
                $this->_hasSessionId = App::$get[$name] != '';
            } else {
     ...
     * This is a wrapper for [PHP session_save_path()](https://secure.php.net/manual/en/function.session-save-path.php).
     * @param string $path the current session save path. This can be either a directory name or a [path alias](guide:concept-aliases).
    ...
    protected function freeze()
    {
        if ($this->getIsActive()) {
            ...
            $this->close();
            App::debug('Session frozen', __METHOD__);
        }
    }
    ...
    protected function unfreeze()
    {
        if (null !== $this->frozenSessionData) {
            ...
            if ($this->getIsActive()) {
                App::debug('Session unfrozen', __METHOD__);
            } else {
                $error = error_get_last();
