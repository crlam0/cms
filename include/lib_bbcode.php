...
function bb_parse($string) {
	...
	while (preg_match_all('`\[(.+?)=?(.*?)\](.+?)\[/\1\]`', $string, $matches)) foreach ($matches[0] as $key => $match) {
		...
		switch ($tag) {
			...
			case 'left': $replacement = "<p align=left>$innertext</p>"; break;
			case 'center': $replacement = "<p align=center>$innertext</p>"; break;
			case 'right': $replacement = "<p align=right>$innertext</p>"; break;
