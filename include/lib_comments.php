...
class COMMENTS
{
    ...
    function show_form($tags = array ()) {
        global $session,$SUBDIR,$editor,$input,$server;
        if ( $this->__new_form ) {
    ...
    function get_form_data($input){
        ...
        if ($input["add_comment"]) { 
            ...
            if (strlen($input[form]["author"]) < 3) {
                ...
                $err = 1;
            } elseif ( ($input["img_code"] != $session["IMG_CODE"]) && (!$settings["debug"]) ) {
            ...
            if ( $err ) {
                ...
                $msg.="Сообщение:\n";
                $msg.=$content."\n";
                if(!$settings["debug"])send_mail($settings["email_to_addr"], "На сайте http://".$server["HTTP_HOST"].$SUBDIR." оставлен новый комментарий.", $msg);
                $this->__new_form = true;
            }
