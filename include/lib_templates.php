...
function tpl_parse_string($content,$tags=array(),$sql_row=array(),$sql_row_summ=array(),$inner_content=""){
	...
	if ($total) while ($temp[$a]){
		...
		if(strlen($tagparam))$tagparam=str_replace(")","",$tagparam);
		//echo "Tag: ".$temp[$a][1]." Class: $tagclass Func: $tagparam <br>";
		if($tagclass=="func"){
			if(strstr($tagparam,",")){$param=explode(",",$tagparam);}else{$param[0]=$tagparam;}
			...
			if(strstr($tagparam,",")){$param=explode(",",$tagparam);}else{$param[0]=$tagparam;$param[1]="";}
			if($param[1]=="nl2br"){
			...
			if(file_exists($tagparam))$fname=$tagparam;
			if(file_exists($DIR.$tagparam))$fname=$DIR.$tagparam;
			if(strlen($fname)){
				ob_start();
				include_once($fname);
		...
		if(isset($replace_str))$content=str_replace("[%".$temp[$a][1]."%]",$replace_str,$content);
		$a++;
	}
	return $content;
}
...
function tpl_parse($content,$tags=array(),$sql_result=array(),$inner_content=""){
	...
	$strings=explode("\n",$content);$loop_start=0;$loop_content="";$result="";
	foreach ($strings as $key => $value){
		if (strstr($value,"[%loop_begin%]")){
			$loop_start=1;
		}elseif (strstr($value,"[%loop_end%]")){
			unset($mysql_row_summ);
			if($sql_result)while($row=$sql_result->fetch_array()){
				$result.=tpl_parse_string($loop_content,$tags,$row,$inner_content)."\n";
				foreach ($row as $key => $value)if((is_double($value))or(is_numeric($value)))$mysql_row_summ[$key]=+$value;
			}
			$loop_start=0;
		}elseif($loop_start){
			$loop_content.=$value;
...
function get_tpl_by_title($title,$tags=array(),$sql_result=array(),$inner_content=""){
	...
	if($template[file_name]){
		$fname="";
		if (file_exists($template[file_name]))$fname=$template[file_name];
		if (file_exists($DIR.$template[file_name]))$fname=$DIR.$template[file_name];
		if($fname){
