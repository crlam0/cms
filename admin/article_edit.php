...
if ($input['view_article']) {
    $_SESSION['view_articles'] = $input['id'];
}

if ($input['view_list']) {
    unset($_SESSION['view_articles']);
}

if (isset($_SESSION['view_articles']) && $_SESSION['view_articles']) {
    list($list_title) = my_select_row("select title from article_list where id='" . $_SESSION['view_articles'] . "'", 1);
...
if ($input['added_article']) {
    ...
    $input['form']['list_id'] = $_SESSION['view_articles'];
