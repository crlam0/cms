...
{    
    ...
    public function __construct() {
        parent::__construct();
        $this->table = 'offers';
        $this->image_path = App::$settings['offers']['upload_path'] ?? 'upload/';
        $this->image_width = App::$settings['offers']['image_width'] ?? 512;
        $this->image_height = App::$settings['offers']['image_height'] ?? 512;
    ...
    public function actionCreate(): string 
    {
        ...
        return $this->render('offers_form.html.twig', $tags);
    }
    ...
    private function deleteImageFile(int $item_id): string {
        list($image_old) = App::$db->getRow("select file_name from {$this->table} where id=?", ['id' => $item_id]);
