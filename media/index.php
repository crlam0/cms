...
if (isset($input["uri"])) {
    ...
    list($session["view_files"]) = $result->fetch_array();

    if (strlen($params[1])) {
        $session["media_page"] = $params[1];
    } else {
        $session["media_page"] = 1;
    }
}
...
if (!isset($session["media_page"]))
    $session["media_page"] = 1;

if (isset($input["page"])) {
    $session["media_page"] = $input["page"];
}
...
if ($session["view_files"]) {
    list($PAGES) = my_select_row("SELECT ceiling(count(id)/$settings[media_file_per_page]) from media_file where list_id=" . $session["view_files"], 1);
    list($title) = my_select_row("select title from media_list where id=" . $session["view_files"], 1);
    $tags[Header] = $title;
    ...
    if ($PAGES > 1) {
        ...
        for ($i = 1; $i <= $PAGES; $i++) {
            if ($i == $session["media_page"]) {
                $tags[pages_list].= "[ <b>$i</b> ]&nbsp;";
            } else {
                $tags[pages_list].= "[ <a href=" . $SUBDIR . get_media_list_href($session["view_files"]) . "$i/>$i</a> ]&nbsp;";
            }
        }
        $tags[pages_list].="</center><br>";
    }
    $offset = $settings[media_file_per_page] * ($session["media_page"] - 1);
    $query = "SELECT * from media_file where list_id=" . $session["view_files"] . " order by date_add asc limit $offset,$settings[media_file_per_page]";
