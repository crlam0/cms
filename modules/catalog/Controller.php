...
$IMG_ITEM_URL  = App::$SUBDIR . App::$settings['catalog_item_img_path'];
$IMG_PART_PATH = App::$DIR    . App::$settings['catalog_part_img_path'];
$IMG_PART_URL  = App::$SUBDIR . App::$settings['catalog_part_img_path'];
...
{ 
    ...
    private function getBackButton($part_id) {
        if ($part_id) {
            list($href_id) = my_select_row("select prev_id from cat_part where id='{$part_id}'", true);
            return '
            <div class="cat_back">
    ...
    private function getPartItemsContent($part_id, $page) {
        ...
        $row_part = my_select_row("select * from cat_part where id='{$part_id}'");
        ...
        list($total) = my_select_row("SELECT count(id) from cat_item where part_id='" . $part_id . "'");
        ...
        if ($result->num_rows) {
            ...
            $content .= get_tpl_by_name('cat_item_list_empty.html.twig', $tags);
        }        
        return $content;
    }
    ...
    private function getRelatedProducts($row_part) {        
        ...
        if(count($related_products)) {
            ...
            $result = my_query($query);
            if ($result->num_rows) {
                return get_tpl_by_name('cat_item_list', $tags, $result);
            }
        }
        return '';
    }
    
    public function actionPartItem($uri, $item_title)
    {        
        list($part_id, $page) = $this->parseURI($uri);
        ...
        if (!$result->num_rows) {
            ...
            return my_msg_to_str('notice', [], 'Товар не найден');
        }
        ...
        return get_tpl_by_name('cat_item_view', $tags, $result);        
    }
    
    public function actionLoadImage()
    {
        list($default_img,$default_img_fname,$title)=my_select_row("select default_img,fname,cat_item.title from cat_item left join cat_item_images on (cat_item_images.id=default_img) where cat_item.id='".$input['item_id']."'");

        $nav_ins = '';
        $input = App::$input;

        list($prev_id,$fname) = my_select_row("select id,fname from cat_item_images where item_id='" . $input['item_id'] . "' and id<'" . $input['image_id'] . "' and id<>'{$default_img}' order by id desc limit 1");
        if ($input['image_id'] != $default_img){
            ...
            list($next_id,$fname) = my_select_row("select id,fname from cat_item_images where item_id='" . $input['item_id'] . "' and id>'" . $input['image_id'] . "' and id<>'{$default_img}' order by id asc limit 1");
            if ($next_id) {
                $nav_ins.= "<a image_id={$next_id} item_id={$input['item_id']} file_name={$fname} class=\"cat_image_button btn btn-default\">Следующая >></a>";
            }
        }else{
            list($next_id,$fname) = my_select_row("select id,fname from cat_item_images where item_id='" . $input['item_id'] . "' and id<>'{$default_img}' order by id asc limit 1");
            if ($next_id) {
                $nav_ins.= "<a image_id={$next_id} item_id={$input['item_id']} file_name={$fname} class=\"cat_image_button btn btn-default\">Следующая >></a>";
            }
        }
