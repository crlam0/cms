...
if($input["added"]){	
	...
	if(($err)||(!$code_ok)){
		...
		$result=my_query($query,$conn);		
                ...
                $msg.="IP: ".$input[form][ip]."\n";
                send_mail($settings["email_to_addr"], "Message from site ".$BASE_HREF, $msg );
		$list=1;
	}
}
...
if($PAGES>1){
	...
	for($i=1;$i<=$PAGES;$i++)$tags[pages_list].=($i==$_SESSION["FAQ_PAGE"]?"[ <b>$i</b> ]&nbsp;":"[ <a href=".$_SERVER["PHP_SELF"]."?page=$i>$i</a> ]&nbsp;");
	$tags[pages_list].="</center>";
}
